...
- 1s大概*$10^8$*循环量,数据量较大的容器遍历可能超时,数据随机可以尝试暴力求解,找到结果后立即推出玄幻
...
-   `abs()`. `sqrt()`.
-   `round()`:四舍五入化为整数,`floor()`向下取整,`ceil()`向上取整
-  `min(a,b) , max({a,b,c})`:多个数找最值
...
6. 周期性边界处理
   - "**轮流操作**": 利用数学偏移思想,计算*周期结束前的最后一步*, 而不是下一个周期的第零步[小L的游戏](https://ac.nowcoder.com/acm/contest/120566/K)
...
|  7   |         a\^a\^a\^a\^...\^a\^a\^B = B          | 找出一堆相同数中不同的数                                     |
|  8   |                     a<<=b                     | 把a的二进制序列向左位移b位,即$a*2^b$                         |
|  9   |                   x \| y==y                   | 说明二进制序列中,x为1的位置y都为1                            |
|  10  |                 num >> j & 1                  | 获取 num 的第 j 位编号 ( j: 0->n )                           |
|  11  |                 a & ~(1 << b)                 | 将 a 的第 b 位设置为 0                                       |
|  12  |                 a \| (1 << b)                 | 将 a 的第 b 位设置为 1                                       |
|  13  |                 a ^ (1 << b)                  | 将 a 的第 b 位取反                                           |
|  14  |                 a & (1 << b)                  | 获取 a 的第 b 位编号                                         |
|  15  |            bool f = ((x ^ y) < 0);            | 判断 x, y 是否异号                                           |
|  17  |             $G(i)=i\oplus(i>>1)$              | 自然二进制转**格雷码 ( 相邻的两个数值在二进制表示中只有一位不同 ,相邻两项异或值之和最小 )** |
|  18  | $B_i=G_n\oplus G_{n-1}\oplus\cdots\oplus G_i$ | 格雷码转自然二进制                                           |
...
5. <u>连通块</u>: 在网格中，若两个坐标间的曼哈顿距离为 1 则视为相邻。由数值相等的格子按该相邻关系划分的极大连通子集称为一个连通块。对于网格中的两个点$(x_1,x_2)$和$(x_2,y_2)$, 其曼哈顿距离为$|x_1-x_2|+|y_1-y_2|$
...
- ```C++
  ...
  min_element(v.begin(),v.end());	// 返回指向最小值的迭代器,本质是O(n)的遍历
...
- **sort** ( 排序 )
  ...
  sort(v.begin(),v.end()); // 升序排序
  	sort(v.begin(),v.end(),greater<T>);	// 降序排序,不推荐
  sort(v.rbegin(),v.rend());//效率高,推荐使用
...
- ```C++
  ...
  v.emplace_back(ele); // 末尾增添元素,可节省调用构造函数的时间,支持多参数
  v.pop_back();	// 删除最后一个元素
...
- 声明
  ...
  ```C++
  priority_queue<T>pq;	// 大根堆,最大值先出队
  priority_queue<T, vector<T>, greater<T> > pq; // 小根堆,最小值先出队
...
- ```C++
  ...
  int getPre(int x){	//TODO 实现找前驱
      auto it = mset.lower_bound(x);// 找到第一个> =x 的元素
      if(it == mset.begin()) return -1;	
      else return *prev(it);// 找到第一个> =x 的元素,prev()返回it的前一个迭代器,也可以用*(--it),返回第一个比x小的数
  }
  
  int getBack(int x){	//TODO 实现找后继
      auto it = mset.upper_bound(x);// 找到第一个 >x 的元素
...
4. **打表**: 用于填空题, 暴力模拟题目要求, 人工寻找输出结果, 一般运行时间*t>1s*
...
- 一般模板:
  ...
  vector<int> ans(n);
  for (int i = 0; i < n; i++){
      while (!stk.empty() && a[stk.top()] < a[i]){//找到第一个比当前值大的元素
          ans[stk.top()] = i + 1;//存放该元素的下标
          //有时为ans[stk.top()]i-stk.top()存放距离
          stk.pop();
      }
      stk.push(i);
  }
...
| 题目 | [第一场 ( 7/12)](https://ac.nowcoder.com/acm/contest/120561) | [第二场 ( 6/10)](https://ac.nowcoder.com/acm/contest/120562) | [第三场 ( 7/10)](https://ac.nowcoder.com/acm/contest/120563) | [第四场 ( 7/10)](https://ac.nowcoder.com/acm/contest/120564) | [第五场 ( 6/10)](https://ac.nowcoder.com/acm/contest/120565) | [第六场 ( 3/10)](https://ac.nowcoder.com/acm/contest/120566#question) |
| ---- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| A    | 状压、枚举、概率、逆元                                       | 简单思维                                                     | 枚举、简单数学                                               | 签到、枚举                                                   | **二进制、二分、lowbit、注意力题**                           | 贪心,  堆/优先队列, 数学、凸函数                             |
| B    | 贪心、思维、数学                                             | 排序、贪心                                                   | 暴力、概率、gcd                                              | 前缀和                                                       | 签到                                                         | **组合数学, 预处理、隔板法**                                 |
| C    | 贪心、脑筋急转弯                                             | **搜索、二分**                                               | 构造、前缀和、Kadane                                         | 构造、位运算、格雷码、分治                                   | **几何、二分，线段覆盖**                                     | **线段树,  动态规划, 区间查询, 点更新**                      |
| D    | **二分、贪心**                                               | **树形 DP、数学**                                            | **网格 BFS、最短路**                                         | **数学、扩展欧几里得（EXGCD）、二分**                        | **贪心、排序、双端队列**                                     | **最短路, Dijkstra, 网格图, 多源最短路**                     |
| E    | 枚举、贪心                                                   | 构造                                                         | **最小生成树、Kruskal**                                      | **状压DP、路径还原、打表**                                   | 贪心、二分，数据结构                                         | **并查集,  离线处理, 排序, 动态连通性、逆向思维**            |
| F    | **组合数学、链式并查集**                                     | 数论、位运算                                                 | 博弈、最短路、构造                                           | 构造                                                         | 贪心、暴力，背包                                             | **组合数学, 容斥原理, 期望, 快速幂, 逆元**                   |
| G    | 分类讨论、枚举、贪心                                         | **数据结构、平衡树**                                         | 贪心、排序、前缀和                                           | 数学、打表、质因数、搜索                                     | 模拟                                                         | 前缀和,  双指针, 二分查找, 尺取法                            |
| H    | **位运算、前缀和、优化 DP**                                  | 计数、容斥                                                   | 解析几何、三角形面积                                         | STL、暴力                                                    | **棋盘染色、贪心**                                           | 动态规划,  位运算, 可达性DP                                  |
| I    | **贪心、位运算、构造**                                       | 思维                                                         | **位运算  (XOR)、线性基**                                    | 概率、期望、诈骗                                             | 二分、前缀和差分                                             | **构造,  数论, 贪心、最大公约数**                            |
| J    | **最小生成树、线段树**                                       | **图论、BFS、最短路**                                        | 完全二叉树编号、位运算                                       | **最短路、路径还原、模拟**                                   | 签到                                                         | **快速傅里叶变换（FFT）/快速数论变换（NTT）, 字符串, 数学,  多项式、卷积** |
| K    | 签到、构造                                                   | (未列出)                                                     | (未列出)                                                     | (未列出)                                                     | (未列出)                                                     | 数学、周期性, 模运算                                         |
| L    | 签到、语法题                                                 |                                                              |                                                              |                                                              |                                                              | **动态规划, 矩阵乘法, 组合数学、Kitamasa算法**               |
